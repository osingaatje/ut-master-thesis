...
Given the existing works mentioned in @relatedwork and @tbl:grader-suitability, the best approach for maximising accuracy, consistency, and grading transparency seems to be to use graph isomorphism algorithms akin to those suggested by #cite(<smith2013>, form: "prose"), implemented by papers such as #cite(<Bian2020>, form: "prose"), #cite(<Hosseinibaghdadabadi2023>, form: "prose"), and #cite(<thomas2009>, form: "prose"). Visualisations similar to @fig:Bian2020_Fig9 could prove to be a nice addition, so architectural support for visualisations will be taken into account, which can be implemented should there be time left.

Unfortunately, no solutions seem to support the integration of ILOs into their grading rubric inputs. While we believe that this is a vital point to consider when making rubrics or example solutions @osinga2024, we realise that it may require extra work for a teacher to add metrics on how much a certain ILO is tested, and may not be compatible with existing rubrics. Therefore, we make this feature optional, and aim to add support linking ILOs either per diagram feature or for specific parts of an example solution. For example, a teacher would ideally be able to mark that the presence of certain classes, certain associations, the multiplicity of associations, or the connection between (certain) classes satisfies a certain set of ILOs.
...
#place(bottom+center, dy: -1em, scope: "column", float: true, [
...
Finally, a query-based architecture allows for caching all stages of the process, which is possible since we make the explicit choice to use only deterministic algorithms. This allows for efficiency improvements if we need to fetch some query output multiple times, for example, if we need to grade a solution we have already seen before.
...
= Planning <planning>
We plan to develop #seshat according to the Agile methodology @agilemanifesto. This means that we divide the work up into increments, and aim to show new deliverables frequently. This prioritises prototyping and frequent feedback, allowing both the author and the supervisors to steer the direction of the project effectively.

We divide these increments up into two weeks. This should allow for enough time inbetween to make significant progress on #seshat and the final paper, while keeping increments small enough to be able to reflect on the progress made often enough and make adjustments to the plan if necessary. We meet with the main supervisor every increment. We invite the co-supervisor to every meeting: they are free to attend when they wish to see progress and/or give advice. When in doubt, we explicitly ask advice of both supervisors to get advice from multiple perspectives.

During the development of #seshat, we develop the paper in parallel, documenting design decisions and progress.

The increments are initially structured in the way defined in @fig:planning. Note that these are subject to change. There is also a bit of buffer time planned in, as it might turn out there is more research needed to complete certain algorithms .
