...
#abstr(content: 
  [
    During computer science studies, students are often required to submit diagrams. The grading of these diagrams is currently done by humans, resulting in a costly, lengthy, and error-prone process. In this paper, we investigate the theoretical feasibility of automatically grading diagrams, focusing on Unified Modelling Lanuage diagrams and the UTML file format used by diagram software at the University of Twente. We examine existing work, which shows that graph isomorphism algorithms which account for the use of synonyms and the presence of spelling mistakes provide the best results, but that existing work does not provide the means necessary to adapt their solutions to the UTML format. Based on these findings, we propose #seshat, an autograder that combines the aforementioned techniques into a generic autograder capable of supporting multiple diagram variants and file formats, with built-in support for UTML. In the final thesis, we realise #seshat and compare it to human grading for multiple UTML exam submission datasets.
  ]
)
...
However, the grading of these diagrams is often a costly and lengthy process, involving multiple paid members of staff @Ahmed2024#footnote("From personal experience.")<footnote:pers-exp>. Additionally, this process is prone to grading inconsistencies due to various reasons @Ahmed2024, the main one being the inherent inconsistency of human graders @Ahmed2024 @Meadows2005. #cite(<Meadows2005>, form: "prose") pose two possible solutions to the problem of human grading: either "report the level of reliability associated with marks/grades, or find alternatives to [grading]." We propose a third alternative: finding alternatives to the grading _process_.
...
#rq([
*RQ2*: To what extent are existing solutions suitable for use in autograding UTML diagrams with regards to (1) accuracy, (2) consistency, (3) grading transparency, (4) extent of linking ILOs to grading instructions, (5) UTML support, and (6) ease of extending the source code to alter functionality?
])
...
*RQ1* and *RQ2* are answered in @relatedwork and @tbl:grader-suitability, by analysing these works for suitability of grading. @tools-techniques explains the plan for building the autograder and @planning outlines the planning for *RQ3* and *RQ4*, which are to be answered in the final thesis, where we grade UTML diagrams using an implementation based on related work and compare it to human grading.

= Related work <relatedwork>
In order to answer research questions *RQ1* and *RQ2*, we conduct a small-scale literature study covering roughly 40 works. It aims to provide an exploratory view into the world of autograders, which means that formal inclusion and exclusion criteria are not set up. Works are collected using the search engines Google Scholar#footnote(link("https://scholar.google.com")) and ResearchGate#footnote(link("https://www.researchgate.net")), using terms including but not limited to "automatically grading UML diagrams", "autograder diagram", "UML diagram assessment", "machine learning diagrams", and "diagram evaluation assessment AI". For ILO research, terms were used such as "learning outcomes include in rubric", "learning objectives in rubrics", and similar. Snowballing (the practice of looking at sources of sources) was used to a depth of 1.

== Autograders
Multiple methods and types of diagrams are researched, including proposed frameworks for autograders, purely algorithmic implementations, and AI-based methods (using Machine Learning (ML) / Generative AI (GenAI) techniques). Additionally, work on integrating ILOs is researched.
...
#cite(<smith2004>, form: "prose") provide a five-step framework for assessing "possibly ill-formed or inaccurate diagrams" that include the steps (1) segmentation, (2) assimilation, (3) identification, (4) aggregation, and (5) interpretation. While the first two steps are meant for translating images or other "raster-based input" into diagrammatic primitives which is not useful for us, the latter stages provide a solid foundation to grade diagrams, which is used by other papers @thomas2009.
...
#cite(<Bian2020>, form: "prose") expand their previous work @Bian2019 (see @subsec:relatedwork-autograder-frameworks) with a case study, using the Levenstein distance between words for syntactic matching, several metrics for semantic matching, and structural matching based on similar attributes and operations within classes. They find that multiple teacher solutions result in more accurate grades with an average accuracy of more than 95% #cite(<Bian2020>, supplement: "p.10"). Additionally, they find that grading configurations need to change per exam if you want similar grades to the teacher, likely because the focus of each exam is on a different aspect of diagram creation (associations, inheritance, etc.). Lastly, they state that autograding "has shown to be more consistent and able to ensure fairness in the grading process" #cite(<Bian2020>, supplement: "p.11"). Additionally, their visual feedback system seems to be a nice addition for seeing exactly where marks were awarded / deducted on the solution diagram (see @fig:Bian2020_Fig9).
...
In conclusion, while AI-based grading has been attempted in recent years, purely AI-driven solutions produce lacking similarity to human grading compared to graph isomorphism-based solutions as well as introducing fundamental non-deterministic behaviour and providing no guarantees for grading consistency. This makes these types of solutions inferior to graph isomorphism solutions in terms of accuracy, consistency, and grading transparency. When used only for semantic and/or syntactic matching, it may provide similar accuracy to algorithmic solutions, although it still introduces nondetermism in grading which negatively affects consistency.

// line break for readability
== Intended Learning Objectives \ and examination
#cite(<osinga2024>, form: "prose") mentions that the top-down approach of defining Intended Learning Objectives, translating them into exercises, and then constructing analytical grading rubrics for these exercises seems to produce exercises and grading rubrics that relate most to a module's ILOs. This is supported by #cite(<dinur2009>, form: "prose"), which state that analytical rubrics (those which mention explicit criteria) provide more details than global, holistic rubrics. Given that rubrics and exercises are defined in such a way, one could encode these ILOs into the grading rubric and show these in the final grade, to indicate to students how well they achieved the learning goals of the module in order to improve the transparency of grading.


== Conclusion
In the explored related work, existing frameworks primarily recommend structural matching in combination with syntactic and semantic matching to be able to effectively grade solutions containing spelling mistakes and the use of synonyms. Existing implementations mostly use the methods recommended by the frameworks, with the best results stemming from determinstic, graph isomorphism algorithms, albeit at the cost of the teacher having to produce one or more sample solutions. Purely AI-driven methods require less effort from teachers, since teachers do not need to produce sample solution(s), but produce noticeably inferior results to graph matching algorithms. Using hybrid methods, specifically using AI-driven classification algorithms for semantic/syntactic matching and graph isomorphism for structural matching, seems to produce similar results to 'pure' graph matching algorithms, but does not necessarily provide accuracy gains over algorithmic solutions and can additionally introduce nondeterminism in otherwise determinstic solutions, reducing consistency.
...
Given the existing works mentioned in @relatedwork and @tbl:grader-suitability, the best approach for maximising accuracy, consistency, and grading transparency seems to be to use graph isomorphism algorithms akin to those suggested by #cite(<smith2013>, form: "prose"), implemented by #cite(<Bian2020>, form: "prose") and #cite(<thomas2009>, form: "prose"). Visualisations similar to @fig:Bian2020_Fig9 could prove to be a nice addition, so architectural support for visualisations will be taken into account, which can be implemented if there is enough time.
...
#place(bottom+center, scope: "column", dy: -2.9em, float: true, [
